...
class qa_select_vectors_vbb(gr_unittest.TestCase):
	"""
	...
	def test_001_select_vectors_vbb(self):
		skip = 2
		len  = 3
		vlen = 2
		...
		select_vectors_vbb = dab_swig.select_vectors_vbb(vlen,len,skip)
		v2s = gr.vector_to_stream(gr.sizeof_char, 2)
		dst = gr.vector_sink_b()
		trigdst = gr.vector_sink_b()
		self.tb.connect(src, s2v, select_vectors_vbb, v2s, dst)
		self.tb.connect(trigsrc, (select_vectors_vbb,1), trigdst)
		self.tb.run()
